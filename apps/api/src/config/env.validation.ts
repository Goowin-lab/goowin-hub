...
const requireValue = (config: Environment, key: string, errors: string[]): void => {
  if (!config[key] || config[key]?.trim() === '') {
    errors.push(`${key} is required`);
  }
};

const validateNumber = (config: Environment, key: string, errors: string[]): void => {
...
export const validateEnvironment = (config: Environment): Environment => {
  ...
  if (!allowedEnvironments.has(nodeEnv)) {
    errors.push(`NODE_ENV must be one of: ${Array.from(allowedEnvironments).join(', ')}`);
  }

  if (config.API_GLOBAL_PREFIX !== undefined && config.API_GLOBAL_PREFIX.trim() === '') {
